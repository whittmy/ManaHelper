...
void HttpRequestor::addTask(PtrRequestInfo info){
    _logger->info("addTask "+ info->url);
    //url.setEncodedUrl()
    QNetworkRequest request(QUrl(info->url));
    QNetworkReply *reply = m_manager->get(request);

    QHash<QNetworkReply*,PtrRequestInfo>::iterator i =  mMap.insert(reply, info);
...
void HttpRequestor::slot_replyFinished(QObject* obj){
    ...
    QHash<QNetworkReply*,PtrRequestInfo>::iterator i = mMap.find(reply);
    if(i != mMap.end()){
