    ...
    ,s_downLoadui(new DownLoadUI(this))
{
    ...
    //luokui
    connect(m_tabWidget, SIGNAL(lastTabClosed()),
            this, SLOT(slotLastTabClosed()));
    connect(m_tabWidget, SIGNAL(tabsChanged()),
            this, SLOT(slotTabsChanged()));
...
void BrowserMainWindow::slotLastTabClosed(){
    qDebug() << "slot_lastTabClosed";
    slotHome();
}

//luokui -----  interact the action m_addtask >>>
void BrowserMainWindow::slotTabsChanged(){
    ...
    if(mSiteParser->bSupport(url)){
        qDebug() << "support the "<<url;
        m_addtask->setEnabled(true);
    }

    //qDebug() << m_addtask->isEnabled();
}
...
//device >>
void BrowserMainWindow::initDeviceMonitor(){
    //设备检查
    connect(_devdetector, SIGNAL(devRemoved()),
                    this, SLOT(slotUpdateDevStatus()));
    connect(_devdetector, SIGNAL(devConnected()),
                    this, SLOT(slotUpdateDevStatus()));
    ...
    m_device->setEnabled(!_devdetector->getDevPath().isEmpty());
}

void BrowserMainWindow::slotUpdateDevStatus(){
    _logger->info("--UpdateDevStatus--");
    QString path = _devdetector->getDevPath();
    if(path.isEmpty()){
        _logger->info("disconnect");
    }
    else{
    ...
    m_device->setEnabled(!path.isEmpty());
}

bool BrowserMainWindow::nativeEvent(const QByteArray &eventType, void *message, long *result){
...
//luokui 在该mainwindow->show()之后调用，效果不稳定>>>
void BrowserMainWindow::hideSome(){
    //luokui>>
    m_navigationBar->setMovable(false); //禁止工具栏移动
    //<<
...
void BrowserMainWindow::setupToolBar()
{
    ...
    connect(m_homepage, SIGNAL(triggered()), this, SLOT(slotHome()));
    //<<<

    m_navigationBar->addWidget(m_tabWidget->lineEditStack());
    //luokui add>>
    m_tabWidget->lineEditStack()->setDisabled(true);
    //<<<
    ...
    */
    ...
    connect(m_download, SIGNAL(triggered()), this, SLOT(slotDownLoadUI()));
    //<<<
    ...
    //luokui  placeholder
    m_placeholder = new QAction(this);
    m_placeholder->setDisabled(true);
    m_navigationBar->addAction(m_placeholder);
}
...
{
    if (title.isEmpty()) {
        setWindowTitle(tr("Manage Helper"));
    } else {
#if defined(Q_OS_OSX)
        setWindowTitle(title);
#else
        setWindowTitle(tr("%1 - Manage Helper", "Page title and Browser name").arg(title));
#endif
    }
