...
int DownloadMgr::activeDownloads() const
{
    int count = 0;
...
void DownloadMgr::download(QWebEngineDownloadItem *download)
{
    DownloadWidget *widget = new DownloadWidget(download, this);
    addItem(widget);
}

void DownloadMgr::addItem(DownloadWidget *widget)
{
...
void DownloadMgr::updateRow()
{
    ...
    if (widget->downloadedSuccessfully()
        && removePolicy() == DownloadMgr::SuccessFullDownload) {
        remove = true;
    }
    if (remove)
...
DownloadMgr::RemovePolicy DownloadMgr::removePolicy() const
{
    return m_removePolicy;
}

void DownloadMgr::setRemovePolicy(RemovePolicy policy)
{
...
void DownloadMgr::updateItemCount()
{
    int count = m_downloads.count();
    itemCount->setText(count == 1 ? tr("1 Download") : tr("%1 Downloads").arg(count));
}

DownloadModel::DownloadModel(DownloadMgr *downloadManager, QObject *parent)
